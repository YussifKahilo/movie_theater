...
class FavoritesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<NetworkConnectivityCubit, bool>(
          ...
          builder: (context, state) {
            if (state is GetUserSuccessState) {
                ...
                builder: (context, state) {
                  if (state is GetFavoritesFailedState) {
                    ...
                    return FavoritesList(
                        totalPages: totalPages, movies: movies);
                  } else {
                    return Column(
              ...
              return Column(
                ...
                children: [
                  const CustomText(
                    'You have to login to view you favorites',
                  ),
                  AppSize.s30.spaceH,
                  CustomContainer(
                    onTap: () =>
                        Navigator.pushNamed(context, Routes.loginScreen).then(
                            (value) => value != null
                                ? FavoritesCubit.get(context).getFavorites(1)
                                : null),
                    haveShadows: true,
                    padding:
                        (PaddingValues.p10, PaddingValues.p80).pSymmetricVH,
                    text: 'Login',
                  ),
                  AppSize.s100.spaceH,
                ],
